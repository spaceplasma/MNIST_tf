...
def main(_):
  ...
  # Test trained model
  correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(y_, 1)) # <-- checks to see if we predicted correctly
  
  accuracy = (tf.reduce_mean(tf.cast(correct_prediction, tf.float32)))*100

  print('Accuracy: '+str(sess.run(accuracy, feed_dict={x: mnist.test.images,
